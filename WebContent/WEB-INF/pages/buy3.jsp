					...
					<div class="row  align-items-end">
						...
						<div class="col-md-3">
							<div class="select-wrap">
								...
								v-on:click="search">搜尋</button>
							</div>
						</div>
						<div v-for="house in info"><p>{{house.title}}</p></div>
					</div>
<!-- 				</form> -->
					</div>
			</div>
      ...
      <c:forEach var="hlist" items="${houselist}">
          ...
          <div class="col-md-6 col-lg-4 mb-4">
            <div class="property-entry h-100">
              ...
              <c:if test="${!empty memberBean}">
                <button class="property-favorite"><span class="icon-heart-o"></span></button>
              </c:if>
	   ...
	   <div class="row mb-5">
          ...
          <div class="col-md-6 col-lg-4 mb-4" id="searchhlist">
            <div class="property-entry h-100">
              <a href="housedetail?HOUSEID=" class="property-thumbnail">
                <div class="offer-type-wrap">
                  <span class="offer-type bg-danger">Sale</span>
                </div>
                <img src="data:image/jpeg;base64,{{house.base64image1}}" alt="Image" class="img-fluid">
              </a>
              <div class="p-4 property-body">
              <c:if test="${!empty memberBean}">
                <button class="property-favorite"><span class="icon-heart-o"></span></button>
			...
			methods:{
				search:function(event){
					if(event){
						...
						let dist = document.getElementById("dist").value;
						let addr = document.getElementById("addr").value;
						axios.get("<c:url value='searchhouse'/>"+"?city="+city+"&dist="+dist+"&addr="+addr)
					      .then(response =>{ console.log("rpdt="+response.data);(this.info = response.data)}) 
